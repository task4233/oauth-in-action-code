...
app.post('/approve', function (req, res) {
	...
	if (req.body.approve) {
		if (query.response_type == 'code') {
			...
			if (__.difference(rscope, cscope).length > 0) {
				const urlParsed = buildUrl(query.redirect_uri, {}, qs.stringify({ error: 'invalid_scope' }));
				res.redirect(urlParsed);
				return;
			}
			...
			// DO NOT generate refresh_token here because implicit_grant should generate short-lived token due to security matter.
			let token_response = {
			...
			const urlParsed = buildUrl(query.redirect_uri, {}, qs.stringify(token_response));
